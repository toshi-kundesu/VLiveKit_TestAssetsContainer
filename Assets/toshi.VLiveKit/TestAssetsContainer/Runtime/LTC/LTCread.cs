...
public class LTCread : MonoBehaviour
{
    ...
    [SerializeField] private bool showUI = true; // UIを表示するかどうかのスイッチ
    ...
    private void Start()
    {
        ...
        if (!string.IsNullOrEmpty(initialDeviceName) && Microphone.devices.Contains(initialDeviceName))
        {
            m_LtcAudioInput = Microphone.Start(initialDeviceName, true, DEVICE_REC_LENGTH, 44100);
        }
